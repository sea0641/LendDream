      ...
      <div class="row">
        ...
        <div class="col-lg-9">
          ...
          <div style="display:flex; flex-wrap:wrap;">
			...
			<div class="auto-jsCalendar classic-theme" style="text-align: center;"><p>※대여 가능 기간 : <%= product.getFrom_d() %>~<%= product.getTo_d() %></p></div>
          </div>
            ...
            <% if(userid != null && userid.equals(mylend.getId())){ %>
            	<div class="mylend card p-3 col-lg-6" style="border-color: #6AAFE6">
            		<h3>나의 대여정보</h3><br>
            		<h6>대여기간: <%= mylend.getFrom_d() %> ~ <%= mylend.getTo_d() %></h6><br>
	                <h6>총 대여료: <%= mylend.getPrice1() %>￦</h6><br>
	                <h6>보증금: <%= mylend.getPrice2() %>￦</h6><br>
	                <h6>대여방식: <%= mylend.getDelivery() %></h6>
            	</div>
            <% } else if(userid != null && userid.equals(product.getId())){ %>
            	<div class="mylend card p-3 col-lg-6" style="border-color: #6AAFE6">
            		<h3>대여 신청자 목록</h3><br>
            		<% for(int u=0; u<alllenders.size(); u++){ %>
            		<h6>아이디: <%= alllenders.get(u) %><br> 대여기간: <%= alldates.get(2*u) %> ~ <%= alldates.get(2*u+1) %></h6><br>
            		<% } %>
            	</div>
            <% } %>
              <label class="mt-3">상품명:</label>
              <h3 class="card-title"><%= product.getProd_name() %></h3>
              <label>
              <span class="fa fa-eye" style="margin-right:10px; font-size:13px"><%= product.getView() %></span>
			  <span class="fa fa-user" style="margin-right:10px; font-size:13px"><%= product.getLend_count() %></span>
              </label>
              <br>
              <p style="font-size:12px; color:gray;">#<%= product.getCategory() %></p><br>
              <label>판매자ID:</label>
              <h4 class="card-title"><%= product.getId() %></h4><br>
              <label>하루당 대여가격:</label>
              <h4><%= product.getPrice() %>￦/day</h4><br>
              <label>설명:</label>
