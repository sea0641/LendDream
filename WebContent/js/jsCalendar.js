var jsCalendar = (function(){
    ...
    JsCalendar.prototype._update = function() {
        ...
        // Update month name
        this._elements.month.textContent = this._date.getFullYear()+"년 "+month.name;
        ...
        //판매자가 올린 대여기간 범위벗어난거 표시
        a=1; //일
        b=parseInt(month.name); //월
        c=this._date.getFullYear(); //년
        
    		
        for(i = month.start; i<=month.end; i++){
        	for(n=0; n<allfrom_d.length; n++){
        		if((b == allfrom_m[n] && b == allto_m[n]) && (a >= allfrom_d[n] && a <= allto_d[n])){
    				this._elements.bodyCols[i-1].className = "already-use";	
        		}
        		else if((allfrom_m[n] != allto_m[n]) && ((b == allfrom_m[n] && a >= allfrom_d[n]) || (b == allto_m[n] && a <= allto_d[n]))){
        			this._elements.bodyCols[i-1].className = "already-use";	
        		}
        	}
        	if(b==from_d_month && a<this._now.getDate()){
        		this._elements.bodyCols[i-1].className = "before-today";
        	}
        	if(c<from_d_year || c>to_d_year) {
        		this._elements.bodyCols[i-1].className = "outof-range";
        	}
        	if(b<from_d_month || b>to_d_month){
        		this._elements.bodyCols[i-1].className = "outof-range";
        	}
        	if((b==from_d_month && a<from_d_date)||(b==to_d_month && a>to_d_date)){
        		this._elements.bodyCols[i-1].className = "outof-range";
        	}
        	a++;
        }
